...
def generate():
    ...
    with conn2:
        c2.execute("SELECT * FROM top10s WHERE field1 = :entry ", {'entry': sentry})
    var = c2.fetchone()
    if not bpmbtn:
        bpm=var[5]
    else:
    ...
    with conn2:
        c2.execute("SELECT * FROM top10s ORDER BY ABS(bpm-:bpm), ABS(nrgy-:energy), ABS(dnce-:dance) LIMIT 20", {'bpm': bpm, 'energy': energy, 'dance':dance})
